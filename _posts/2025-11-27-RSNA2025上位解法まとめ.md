...
- モデルのバージョンについてまとめます。Train v6 → OOF 予測を元に Filter v6（怪しいスライスだけの filt1.csv）。Train v8 & v9 → v6 で絞ったデータ＋全体データを使ってより強いモデルを学習しています。また、OOFを使って Filter v8 を作っています。Train v10 & v13 → v8 Filter などを利用して、さらにチューニングしたモデルです。
...
- 各ベストモデルのOOF確率を読み込み、モデル間で平均して教師確率ベクトルを作ります。これをpseudo label としてteacher_prob.npy などに保存します。 teacher_prob を、元ラベルと並んで新しい教師ラベルとして使います。

- 先ほど作ったteacher_probを使って、知識蒸留（distillation）を行います。
MaxViT‑tiny v1、MaxViT‑small v1、CoaT‑Lite‑Medium v3に対してSegmentation Crop で切り抜かれた ROI 画像を入力します。そして、
生の 0/1 ラベル（train_sampled / train.csv）とpseudo teacher の確率 teacher_probを予測します。loss としては以下のように、元ラベル＋teacher を両方見る。
...
- これで、try7 のアンサンブルが持っていた知識をMaxViT / CoaT v3 などより強い構造の 1 モデルに蒸留する＝ try8 の強力な最終モデル群が出来上がります。
 

### Inference Ensemble(ピンク)

- テスト患者に対して、まずViT‑Small‑Plus dinov3 Crop Model を走らせて、bbox（ROI）を推定し、その bboxでテストデータをcrop し、モデル入力画像を作ります。次の 4 系統のモデル（try7 CoaT‑Lite‑Medium v13（fold 2,3）, try8 CoaT‑Lite‑Medium v3（fold 0,1）,MaxViT‑small v1（fold 2,3）, MaxViT‑tiny v1（fold 0,1）※ 図では fold の割り当てが書いてあり、全て合わせると 0〜3 の 4 fold をカバーしています）でからの出力を得ます。

- 各モデルの出力（14 クラスの確率）をアンサンブルして、 最終的な提出用予測を作る。
...
#### 動脈瘤検出(exp0)
- ここでは、重心座標の前後 ±10 スライスを表示し、LabelImg で動脈瘤を囲むbboxを手動で書きます。

- そしてそのbboxを教師データにして、YOLOv11（検出モデル）を学習します。

#### マルチクラス分類 + マルチタスクモデル(exp2/3)
- exp2-clsはViT large / EVA largeの14クラス分類モデルです。陰性例は全スライスで、陽性例はexp0で作ったbboxを含むスライスだけ使って学習しています。

- exp-auxは、MIT-B4 + FPNで構成される分類 + セグメンテーションモデルで、分類ラベルは14クラス、セグメンテーションはexp0のbboxを塗りつぶしてマスクを作成しています。

#### 脳検出モデル (exp1)

- 各シリーズの全スライスを平均して1枚の画像を生成します。その画像に対して脳bboxを人手でアノテーションします。

- YOLO v5nでモデルを学習します。

### 外部データセット

- Lausanne_TOFMRA, Royal_Brisbane_TOFMRA の2つの外部 TOF-MRA データセットを使用します。最初はラベルはなく、どこに動脈瘤があるかはわかりません。

- RSNAで学習済みの、epx2-clsとexp3-aux、exp0-Aneurysm detを使って、外部データセットにpsuedo labelingを施します。前者2つではシリーズレベルのaneurysm有無を予測し、後者ではbboxを予測し、位置を推定します。

### RSNAラベルのクリーニング

- psuedo labelの発想で、RSNAの元トレーニングデータ自体を見直します。

- 具体的には、RSNAのNegativeシリーズのうち、exp2と3sp3の両方で「aneurysm positive > 0.9」を探し、そうシリーズはラベルミスとしてNegativeをPositiveに変更して、exp0のYOLOを使ってbboxを取得してラベルを与えます。

### クリーニングした RSNA データセットと外部 psuedo データセットで再学習

- 先ほど作成したデータセット2つを入力として、exp4-cls, exp5-auxを作成します。どちらも、esp2-clsとexp3-auxの強化版です。

### テストセットと提出

- Kaggleのテストデータに対して、4つのモデルで予測を出します。これをアンサンブルして、最終的な予測とします
