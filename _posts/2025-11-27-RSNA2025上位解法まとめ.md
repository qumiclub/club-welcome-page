...
各ベストモデルの OOF 確率（[N,14]）を読み込む。
モデル間で平均（または重み付き平均）して
Python
teacher_prob[i, c] = 平均予測確率
という 教師確率ベクトル を作る。
これを pseudo label として teacher_prob.npy などに保存する。
「This ensemble's predictions are used as new, refined labels」
→ teacher_prob を、元ラベルと並んで**新しい教師（soft label）**として使う。
3‑3. Second Stage Model Training (try8)
ここが 知識蒸留（distillation）ステージです。
モデル：
MaxViT‑tiny v1
MaxViT‑small v1
CoaT‑Lite‑Medium v3
...
入力：Segmentation Crop で切り抜かれた ROI 画像。
ターゲット：
生の 0/1 ラベル（train_sampled / train.csv）
pseudo teacher の確率 teacher_prob
loss 例：
Python
loss = BCE_with_logits(logits, hard_targets) \
     + λ * KL(sigmoid(logits) || teacher_prob)
のように、元ラベル＋teacher を両方見る。
これで、

try7 のアンサンブルが持っていた知識を
MaxViT / CoaT v3 などより強い構造の 1 モデルに蒸留する
＝ try8 の強力な最終モデル群が出来上がります。
...
- テスト患者に対して、まず
ViT‑Small‑Plus dinov3 Crop Model を走らせて、bbox（ROI）を推定。

その bbox でテスト CT を crop し、モデル入力画像を作る。

次の 4 系統のモデルで予測を出す：

try7 CoaT‑Lite‑Medium v13（fold 2,3）
try8 CoaT‑Lite‑Medium v3（fold 0,1）
MaxViT‑small v1（fold 2,3）
MaxViT‑tiny v1（fold 0,1）
※ 図では fold の割り当てが書いてあり、全て合わせると 0〜3 の 4 fold をカバー。

各モデルの出力（14 クラスの確率）をアンサンブル（平均など）して、 最終的な提出用予測を作る。
